...
int main() {
	...
	cout << "beforee making the arrays" << endl;
	...
	for (int i = 0; i < arrSize; i++) {
		j = distrib(gen);
		mainArray[i] = j; // the array is filled with 65536 random values from 0 to 32 bits
	}
	...
	//cout << "before thread loop" << endl;
		//for (int i = 0; i < forLoopSize; i++) {
			...
			int *a = (int*)aligned_alloc(64, sizeof(int) * endIndex);
			int *output = (int*)aligned_alloc(64, sizeof(int) * endIndex);
			a = mainArray;
			output = outputMain;
