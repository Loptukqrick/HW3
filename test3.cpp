		...
		//output = outputMain;
			...
			while (sortedBlockSize < endingSortedBlockSize) {
				...
				for (int arrIndex = startIndex; arrIndex < endIndex; arrIndex += sortedBlockSize * 8) {
					...
					A1in = _mm512_load_si512(&innerArr[startA1]); // first 16 integers <-- loading in vectors
					cout << "A1in" << endl;
					A2in = _mm512_load_si512(&innerArr[startA2]); // second 16 integers <-- loading in vectors
					cout << "A2in" << endl;

					A1out; // Become inputs of next call
					A2out; // Become inputs of next call

					B1in = _mm512_load_si512(&innerArr[startB1]);
					cout << "B1in" << endl;
					B2in = _mm512_load_si512(&innerArr[startB2]);
					cout << "B2in" << endl;
					B1out;
					B2out;

					C1in = _mm512_load_si512(&innerArr[startC1]);
					cout << "C1in" << endl;
					C2in = _mm512_load_si512(&innerArr[startC2]);
					cout << "C2in" << endl;
					C1out;
					C2out;

					D1in = _mm512_load_si512(&innerArr[startD1]);
					cout << "D1in" << endl;
					D2in = _mm512_load_si512(&innerArr[startD2]);
					cout << "D2in" << endl;
					D1out;
					D2out;
