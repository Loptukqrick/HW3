		...
		//output = outputMain;
			...
			while (sortedBlockSize < endingSortedBlockSize) {
				...
				for (int arrIndex = startIndex; arrIndex < endIndex; arrIndex += sortedBlockSize * 8) {
					...
					A1in = _mm512_load_si512(&[startA1]); // first 16 integers <-- loading in vectors
					//cout << "A1in" << endl;
					A2in = _mm512_load_si512(&[startA2]); // second 16 integers <-- loading in vectors
					//cout << "A2in" << endl;

					A1out; // Become inputs of next call
					A2out; // Become inputs of next call

					B1in = _mm512_load_si512(&[startB1]);
					//cout << "B1in" << endl;
