		...
		//output = outputMain;
			...
			while (sortedBlockSize < endingSortedBlockSize) {
				...
				for (int arrIndex = startIndex; arrIndex < endIndex; arrIndex += sortedBlockSize * 8) {
					...
					int endA2 = arrIndex + (sortedBlockSize * 2);
					int endB1 = arrIndex + (sortedBlockSize * 3);
					int endB2 = arrIndex + (sortedBlockSize * 4);
					int endC1 = arrIndex + (sortedBlockSize * 5);
					int endC2 = arrIndex + (sortedBlockSize * 6);
					int endD1 = arrIndex + (sortedBlockSize * 7);
					int endD2 = arrIndex + (sortedBlockSize * 8);
					//cout << "arrIndex " << arrIndex << endl;


					// Try this conditional...
					//if (arrIndex < 128) {
					selectionSort(tempIn, startA1, endA1);
					selectionSort(tempIn, startA2, endA2);
					selectionSort(tempIn, startB1, endB1);
					selectionSort(tempIna, startB2, endB2);
					selectionSort(tempIn, startC1, endC1);
					selectionSort(tempIn, startC2, endC2);
					selectionSort(tempIn, startD1, endD1);
					selectionSort(tempIn, startD2, endD2);
					...
					A1in = _mm512_load_si512(&tempIn[startA1]); // first 16 integers <-- loading in vectors
					//cout << "A1in" << endl;
					A2in = _mm512_load_si512(&tempIn[startA2]); // second 16 integers <-- loading in vectors
					//cout << "A2in" << endl;

					A1out; // Become inputs of next call
					A2out; // Become inputs of next call

					B1in = _mm512_load_si512(&tempIn[startB1]);
					//cout << "B1in" << endl;
					B2in = _mm512_load_si512(&tempIn[startB2]);
					//cout << "B2in" << endl;
					B1out;
					B2out;

					C1in = _mm512_load_si512(&tempIn[startC1]);
					//cout << "C1in" << endl;
					C2in = _mm512_load_si512(&tempIn[startC2]);
					//cout << "C2in" << endl;
					C1out;
					C2out;

					D1in = _mm512_load_si512(&tempIn[startD1]);
					//cout << "D1in" << endl;
					D2in = _mm512_load_si512(&tempIn[startD2]);
					//cout << "D2in" << endl;
					D1out;
					D2out;
					...
					for (int j = 0; j < (sortedBlockSize / 8) - 1; j++) { 
						...
						if (j == (sortedBlockSize / 8) - 2) {
							...
							if (startA1 == endA1) {
								...
								A2in = _mm512_load_si512(&tempIn[startA2]);
								startA2 += 16;
							}
							else if (startA2 == endA2) {
								// do something else
								A2in = _mm512_load_si512(&tempIn[startA1]);
								startA1 += 16;
							}
							...
							if (startB1 == endB1) {
								...
								// do something else
								B2in = _mm512_load_si512(&tempIna[startB1]);
								startB1 += 16;
							}
							else if (startB1 < startB2) {
								B2in = _mm512_load_si512(&tempIn[startB1]);
								startB1 += 16;
							}
							else {
								B2in = _mm512_load_si512(&tempIn[startB2]);
							...
							if (startC1 == endC1) {
								...
								// do something else
								C2in = _mm512_load_si512(&tempIn[startC1]);
								startC1 += 16;
							}
							else if (startC1 < startC2) {
								C2in = _mm512_load_si512(&tempIna[startC1]);
								startC1 += 16;
							}
							else {
								C2in = _mm512_load_si512(&tempIn[startC2]);
							...
							if (startD1 == endD1) {
								...
								// do something else
								D2in = _mm512_load_si512(&tempIn[startD1]);
								startD1 += 16;
							}
